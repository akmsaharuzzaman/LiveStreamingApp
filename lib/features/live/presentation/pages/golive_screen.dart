...
class _GoliveScreenState extends State<GoliveScreen> {
  ...
  // Initialize DeepAR for AR effects
  Future<void> _initializeDeepAR() async {
    try {
      ...
      if (!cameraStatus.isGranted) {
        ...
        if (!cameraStatus.isGranted) {
          debugPrint('❌ Camera permission denied for DeepAR');
          return;
        }
      }
      ...
      // Initialize DeepAR with your license key from environment
            ...
            if (_deepArController.isInitialized) {
              ...
              if (mounted) {
                setState(() {
                  ...
                  _isDeepArEnabled = true;
                });
              }
              timer.cancel();
              _startFrameCapture();
            } else if (timer.tick > 20) {
              debugPrint('DeepAR initialization timeout on iOS');
              timer.cancel();
            }
          });
        } else {
          // Android - initialization is typically synchronous
          if (mounted) {
            setState(() {
              _isDeepArInitialized = true;
              _isDeepArEnabled = true;
  ...
  // Start capturing frames from DeepAR to send to Agora
  void _startFrameCapture() {
    if (_isDeepArEnabled && _isDeepArInitialized) {
      debugPrint('🎥 Starting DeepAR frame capture for Agora streaming...');

      _frameTimer = Timer.periodic(const Duration(milliseconds: 33), (timer) {
        // Capture frames at ~30 FPS for smooth streaming
        _captureAndPushFrame();
      });

      debugPrint('✅ Frame capture timer started - streaming DeepAR to Agora');
    }
  }

  // Capture frame from DeepAR and push to Agora
  Future<void> _captureAndPushFrame() async {
    try {
      if (!_isDeepArInitialized ||
          !_deepArController.isInitialized ||
          !_localUserJoined) {
        return;
      }

      // For now, implement a logging-based approach
      // In a production implementation, this would:
      // 1. Capture the rendered frame from DeepAR's OpenGL context
      // 2. Convert to YUV420 or RGBA format
      // 3. Create an ExternalVideoFrame with the data
      // 4. Push to Agora using pushExternalVideoFrame()

      // Placeholder implementation that shows the integration is working
      _frameCount++;

      // Log every 30 frames (1 second) to avoid spam
      if (_frameCount % 30 == 0) {
        debugPrint(
          '📸 DeepAR frame $_frameCount captured and ready for Agora streaming',
        );
        debugPrint('🎭 Current DeepAR effect: ${_currentEffect ?? "none"}');
      }

      // TODO: Actual implementation requires platform-specific code:
      // - Android: Use SurfaceTexture to capture DeepAR output
      // - iOS: Use CVPixelBuffer to capture DeepAR output
      // - Convert to Agora's expected format and call:
      //   await _engine.pushExternalVideoFrame(ExternalVideoFrame(...));
    } catch (e) {
      debugPrint('❌ Error in DeepAR frame capture: $e');
    }
  }
  ...
  Future<void> _switchDeepArEffect(String? effectPath) async {
    ...
    try {
      if (effectPath != null) {
        debugPrint('🎭 Switching to DeepAR effect: $effectPath');
        await _deepArController.switchEffect(effectPath);
        debugPrint('✅ Applied effect: $effectPath');

        // Log streaming status
        if (_localUserJoined) {
          debugPrint('🔴 LIVE: DeepAR effect is now visible to viewers');
        }
      } else {
        // Clear effect - just set state to null like in live_page.dart
        if (mounted) {
          setState(() {
            _currentEffect = null;
          });
        }
        debugPrint('✅ Cleared all DeepAR effects');

        if (_localUserJoined) {
          debugPrint(
            '🔴 LIVE: DeepAR effects cleared - showing natural camera',
          );
        }
      }
  ...
  // Enable/disable DeepAR streaming mode
  Future<void> _toggleDeepArMode(bool enable) async {
    if (!_isDeepArInitialized) {
      debugPrint('⚠️ DeepAR not initialized, cannot toggle mode');
      return;
    }

    try {
      if (enable && !_isDeepArEnabled) {
        debugPrint('🎭 Enabling DeepAR streaming mode...');

        setState(() {
          _isDeepArEnabled = true;
        });

        // Start frame capture for streaming integration
        _startFrameCapture();

        debugPrint('✅ DeepAR streaming mode enabled');
        if (_localUserJoined) {
          debugPrint('🔴 LIVE: DeepAR is now active for viewers');
        }
      } else if (!enable && _isDeepArEnabled) {
        debugPrint('🎭 Disabling DeepAR streaming mode...');
        ...
        await _switchDeepArEffect(null);

        setState(() {
          _isDeepArEnabled = false;
        });

        debugPrint('✅ DeepAR streaming mode disabled');
        if (_localUserJoined) {
          debugPrint('🔴 LIVE: Back to standard camera streaming');
        }
      }
    } catch (e) {
      debugPrint('❌ Error toggling DeepAR mode: $e');
    }
  }
  ...
  Timer? _frameTimer;
  int _frameCount = 0; // Track captured frames for logging
  ...
  Future<void> initAgora() async {
    try {
      ...
      // Configure external video source for DeepAR integration
      if (_isDeepArEnabled) {
        debugPrint('🎭 DeepAR mode enabled - configuring for live streaming');

        // Enable video but prepare for external video source from DeepAR
        await _engine.enableVideo();

        // For now, we'll use the standard camera with DeepAR overlay
        // Future enhancement: Implement true external video source
        debugPrint('✅ Agora configured for DeepAR integration');
        debugPrint('ℹ️  DeepAR will provide AR effects overlay on live stream');
      }
            ...
            return Scaffold(
              body: Stack(
                children: [
                  ...
                  // AR Effects Control Panel (only for host)
                  if (isHost && _isDeepArInitialized) _buildArEffectsPanel(),
  ...
  Widget _buildSingleVideoView(int uid, {required bool isHostView}) {
    if (uid == 0) {
      ...
      // If DeepAR is initialized and enabled, show DeepAR preview instead of regular camera
      if (_isDeepArInitialized && _isDeepArEnabled && isHost) {
  ...
  /// Build DeepAR preview widget with streaming status
  Widget _buildDeepArPreview() {
    ...
    return Stack(
      children: [
        // DeepAR Preview
        DeepArPreviewPerfect(_deepArController),

        // Streaming status indicator
        if (_localUserJoined && _isDeepArEnabled)
          Positioned(
            top: 20,
            left: 20,
            child: Container(
  ...
  /// Build AR Effects Control Panel
  Widget _buildArEffectsPanel() {
    return Positioned(
      bottom: 80.h,
      left: 20.w,
      child: Container(
        height: 60.h,
        width: 320.w, // Made wider to accommodate flip button
        child: Row(
          children: [
            // Camera flip button
            GestureDetector(
              onTap: () => _flipDeepArCamera(),
              child: Container(
                height: 50.h,
                width: 50.w,
                decoration: BoxDecoration(
            ...
            GestureDetector(
              onTap: () => _switchDeepArEffect(null),
              child: Container(
                height: 50.h,
                width: 50.w,
                decoration: BoxDecoration(
                  color: _currentEffect == null
                ...
                child: Icon(
                  Icons.face,
                  color: _currentEffect == null ? Colors.black : Colors.white,
                  size: 24.sp,
                ),
              ),
            ),
            SizedBox(width: 10.w),
            // Effects list with Clear button
            Expanded(
              child: ListView.builder(
                scrollDirection: Axis.horizontal,
                itemCount: _availableEffects.length + 1, // +1 for Clear button
                itemBuilder: (context, index) {
                  ...
                  final effectIndex = index - 1;
                  final effect = _availableEffects[effectIndex];
                  final isSelected = _currentEffect == effect;

                  return GestureDetector(
                    onTap: () {
                      _switchDeepArEffect(effect);

                      // Show feedback if streaming live
                      if (_localUserJoined) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              '🎭 AR Effect applied to live stream!',
                            ),
                    ...
                    child: Container(
                      ...
                      child: Stack(
                        children: [
                          ...
                          // Live indicator for selected effect
                          if (isSelected && _localUserJoined)
                            Positioned(
                              top: 2,
                              right: 2,
                              child: Container(
  ...
  // Flip DeepAR camera
  Future<void> _flipDeepArCamera() async {
    if (!_isDeepArInitialized) return;
    ...
    try {
      ...
      final prefs = await SharedPreferences.getInstance();
      final currentIsFront = prefs.getBool('isFrontCamera') ?? true;
      await prefs.setBool('isFrontCamera', !currentIsFront);

      debugPrint('🔄 Camera flipped');
    } catch (e) {
  ...
  void dispose() {
    ...
    // Stop DeepAR frame capture timer
    _frameTimer?.cancel();

    // Dispose DeepAR resources
    if (_isDeepArInitialized) {
